...
public class UserController {
	...
	ResponseEntity createUser(@RequestBody @Valid @NotNull UserVO userVO) {
//		List<UserEntity> users = userVOs.stream().filter(t-> t!=null).map(t,)map
		this.userService.create(userVO);
		String greetings = "Registration is successful! Welcome "+userVO.getFirstName()+", "+userVO.getLastName()+ "to be one of our family!";
		msgTemplate.convertSendAndReceive(AMQPConfiguration.topicExchangeName, "user.creation.successful", greetings);
		return ResponseEntity.status(HttpStatus.OK).build();
	}
	
	@RequestMapping(path="/create", method=RequestMethod.PUT)
	ResponseEntity createUsers(@RequestBody @Valid @NotNull List<UserVO>userVOs) {
//		List<UserEntity> users = userVOs.stream().filter(t-> t!=null).map(t,)map
